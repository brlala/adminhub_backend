...
async def question_ranking():
    ...
    data = await question_ranking_data(recent, past)
    total_model = await question_ranking_total(recent, past)
    average_model = await question_ranking_average(recent, past)
    return data, total_model, average_model


async def question_ranking_data(recent, past):
    """
    ...
    res = sorted(res, key=lambda x: (-x['count'], -x['trend']))
    return res
...
async def question_ranking_total(recent, past) -> (int, float, list[dict]):
...
async def question_ranking_average(recent, past) -> (int, float, list[dict]):
